...
- (void)processAudioInput:(NSArray *)ringBuffers sampleRate:(Float32)sampleRate allChannels:(BOOL)allChannels
{
    if (allChannels)
    {
        ...
        [ringBuffers.firstObject copyTo:_inputBuffer length:_pointNumber];
        for (NSUInteger i = 1; i < ringBuffers.count; i++)
    ...
    {
        [self processAudioInput:ringBuffers  sampleRate:sampleRate channel:0];
    }
}
...
- (void)processAudioInput:(NSArray *)ringBuffers sampleRate:(Float32)sampleRate channel1:(NSUInteger)channel1 channel2:(NSUInteger)channel2
{
    ...
    [[ringBuffers objectAtIndex:channel1] copyTo:_inputBuffer length:_pointNumber];
